...
class Serial:
    ...
    def get_expert_actions(self):
        ...
        for env in self.envs:
            if hasattr(env, "get_expert_action_at_timestep"):
                # Use current tick as timestep
                timestep = getattr(env, "tick", 0)
                expert_actions, valid_mask = env.get_expert_action_at_timestep(timestep)
...
def _worker_process(
    ...
    # Helper to update expert actions from environment(s)
    def update_expert_actions():
        if hasattr(envs, "get_expert_actions"):
            # Serial backend has get_expert_actions
            exp_act, exp_valid = envs.get_expert_actions()
            expert_actions_arr[:] = exp_act
            expert_valid_arr[:] = exp_valid
        elif hasattr(envs, "get_expert_action_at_timestep"):
            # Native PufferEnv (e.g., Drive) has get_expert_action_at_timestep
            timestep = getattr(envs, "tick", 0)
            exp_act, exp_valid = envs.get_expert_action_at_timestep(timestep)
            expert_actions_arr[:] = exp_act
            expert_valid_arr[:] = exp_valid
